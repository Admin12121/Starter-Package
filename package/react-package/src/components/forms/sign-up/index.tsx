...
const OtpInput = React.lazy(() =>
  import("@/components/global/otp-input").then((component) => ({ default: component.default }))
);
  ...
  <div className="flex py-2 px-1 justify-between">
    <span className="flex items-center gap-2 justify-center">
      ...
      <label htmlFor="terms" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-themeTextGray">
        Accept <Link to="/" className="text-gray-300">Terms</Link> and <Link to="/" className="text-gray-300">Conditions</Link>
      </label>
    </span>
  </div>
);
    ...
    <>
      ...
      <form onSubmit={onInitiateUserRegistration} className="flex flex-col gap-3 mt-10">
        {verifying ? (
          ...
          React_CONSTANTS.signUpForm.map((field) => (
      ...
      {!verifying && <div className="mt-5 w-full relative flex items-center justify-center gap-1">
        ...
        <Link to="/auth/sign-in" className="cursor-pointer text-sm">Sign In</Link>
      </div>}
    </>
  );
};
